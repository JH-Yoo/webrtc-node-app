...
export default class Downloader extends DefaultRtcPC {
  constructor(ws, uploader) {
    ...
    console.log(`[${uploader.getId()}] Downloader :`, this.uploader.sourceStream);

    this.sourceStream = this.uploader.sourceStream.clone();
    this.setMediaStream(this.pc, this.sourceStream, this.senders);
  ...
  setMediaStream(target, stream) {
    stream.getTracks().forEach((track) => {
      this.addSender(track.kind, target.addTrack(track, this.sourceStream));
    });
  }
  setDataChannel() {
